...
class MyClass
{
    ...
    public static function buildFromInput($input, bool $validate = true)
    {
        ...
        $foo = property_exists($input, 'foo') ? (is_int($input->{'foo'}) || is_float($input->{'foo'})) ? (str_contains((string)($input->{'foo'}), '.') ? (float)($input->{'foo'}) : (int)($input->{'foo'})) : ((is_string($input->{'foo'})) ? ($input->{'foo'}) : (null)) : null;
