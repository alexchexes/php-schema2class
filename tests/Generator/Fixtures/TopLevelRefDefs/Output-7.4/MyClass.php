...
namespace Ns\TopLevelRefDefs_7_4;
...
class MyClass
{
    ...
    {
        if (isset($this->foo)) {
            $this->foo = is_object($this->foo) ? clone $this->foo : $this->foo;
        }
        if (isset($this->encoded)) {
            $this->encoded = is_object($this->encoded) ? clone $this->encoded : $this->encoded;
        }
    }
}
