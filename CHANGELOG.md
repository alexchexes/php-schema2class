...
- Setter methods (`withX()`) now accept an optional `$validate` argument to be able skip validation, mirroring `fromInput()`.
 - Setter validation blocks are now generated only when schema constraints are not already enforced by PHP type hints, ensuring consistent and sensible validation coverage.
