...
| `--clean-target-dir`          | `cleanTargetDirectory` (per‑file)  | Remove all files from the target directory before writing new ones. |
...
| `--target-php`, `-p`          | `targetPHPVersion`                 | PHP version **with which** the generated code must be compatible.                                                                                                      |
